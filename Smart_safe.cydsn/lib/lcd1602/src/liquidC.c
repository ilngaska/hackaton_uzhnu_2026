...
static void DelayMC(uint32_t delay)
{
    CyDelayUs(delay);
}
...
static void i2CWrite(uint8_t a)
{
    uint8_t buff[1] = { a};
      (void) I2C_I2CMasterWriteBuf(lcdi2c.Addr,
                                           buff,
                                           1,
...
//	I2C_GenerateSTOP(I2C1, ENABLE); //Wire.endTransmission();
    i2CWrite(_data| lcdi2c.backlightval);
}
...
// Sends a string to the LCD output one character at a time
