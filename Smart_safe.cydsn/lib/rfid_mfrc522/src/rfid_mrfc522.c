...
#define MRF522_SET_RST_STATE 0
#define MRF522_RESET_RST_STATE 1
static rfid_ctx_t mrf522_ctx;
...
u_char Read_MFRC522(u_char addr) {
	return (u_char) mrf522_ctx.read_reg(addr); // return the rx bits, casting to an 8 bit int and chopping off the upper 24 bits
}
//--------------------------------------------------------
/*
 ...
 * Function Description: Add SPI context and init MFRC522
 * Input Parameters ctx - SPI context
 * Return value: None
 */
void MFRC522_init(rfid_ctx_t ctx)
{
  mrf522_ctx = ctx;
  mrf522_ctx.rst(MRF522_SET_RST_STATE);  

  mrf522_ctx.mdelay(100);
  mrf522_ctx.rst(MRF522_RESET_RST_STATE);
